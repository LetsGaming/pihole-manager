    ...
    <template v-else-if="summary">
      <div class="ioc-stats">
        ...
        <div class="ioc-stat">
          ...
          <div class="ioc-stat-value red">{{ fmt(summary.ads_blocked_today) }}</div>
        </div>
        <div class="ioc-stat">
          <div class="ioc-stat-label">BLOCK RATE</div>
          <div class="ioc-stat-value cyan">{{ fmtPct(summary.ads_percentage_today) }}</div>
        </div>
        <div class="ioc-stat">
          <div class="ioc-stat-label">DOMAINS LIST</div>
          <div class="ioc-stat-value">{{ fmt(summary.domains_being_blocked) }}</div>
        </div>
      </div>

      <!-- Blocking toggle -->
      <div class="ioc-footer">
        <div class="ioc-url text-mono text-xs text-muted truncate">{{ instance.url }}</div>
        <label class="toggle toggle-wrap" style="gap:8px;cursor:pointer">
          <input
            type="checkbox"
            :checked="summary.status === 'enabled'"
            @change="$emit('toggle-blocking', instance.id, ($event.target as HTMLInputElement).checked)"
          />
...
.instance-overview-card:hover { border-color: var(--border-dim); transform: translateY(-1px); }
.instance-overview-card.online  { border-top: 2px solid var(--status-online); }
.instance-overview-card.offline { border-top: 2px solid var(--status-offline); opacity: 0.7; }

.ioc-header {
  display: flex; align-items: center; justify-content: space-between;
  padding: 14px 16px 10px; border-bottom: 1px solid var(--border-subtle);
}
.ioc-name { font-weight: 600; font-size: 15px; }
.ioc-offline {
  display: flex; align-items: center; gap: 8px;
  padding: 20px 16px; color: var(--accent-red); font-size: 13px;
}
.ioc-skeleton { padding: 16px; }
.ioc-stats { display: grid; grid-template-columns: 1fr 1fr; padding: 8px 0; }
.ioc-stat {
  padding: 10px 16px;
  border-right: 1px solid var(--border-subtle);
  border-bottom: 1px solid var(--border-subtle);
}
.ioc-stat:nth-child(even) { border-right: none; }
.ioc-stat:nth-last-child(-n+2) { border-bottom: none; }
.ioc-stat-label {
  font-family: var(--font-mono); font-size: 9px; color: var(--text-muted);
  letter-spacing: 0.1em; text-transform: uppercase; margin-bottom: 4px;
}
.ioc-stat-value { font-family: var(--font-mono); font-size: 18px; font-weight: 700; }
.ioc-stat-value.red  { color: var(--accent-red); }
.ioc-stat-value.cyan { color: var(--accent-cyan); }
.ioc-footer {
  display: flex; align-items: center; justify-content: space-between;
  padding: 10px 16px; background: var(--bg-surface); border-top: 1px solid var(--border-subtle);
}
.ioc-url { max-width: 180px; }
</style>
