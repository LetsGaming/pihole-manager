...
function makeClient(getImpl: (() => Promise<unknown>) | object): MockClient {
  const get = typeof getImpl === 'function'
    ? vi.fn(getImpl as () => Promise<unknown>)
    : vi.fn().mockResolvedValue({ data: getImpl });
  const client: MockClient = { get, interceptors: { request: { use: vi.fn((fn: unknown) => fn) } } };
  vi.mocked(axios.create).mockReturnValue(client as never);
  return client;
}

function onlineSummary(overrides: Record<string, unknown> = {}) {
  return { status: 'enabled', dns_queries_today: 1000, ads_blocked_today: 100, ads_percentage_today: 10, domains_being_blocked: 80000, unique_clients: 5, ...overrides };
}
...
describe('Workflow: Add instance and refresh', () => {
  it('adds instance and fetches summary on refreshAll', async () => {
    ...
    store.addInstance({ name: 'Home Pi-hole', url: 'http://pi.hole', apiToken: 'tok', apiVersion: 'v5' });
  ...
  it('marks instance offline when connection fails', async () => {
    ...
    const store = useInstanceStore();
    store.addInstance({ name: 'Dead Pi', url: 'http://dead.local', apiToken: 'tok', apiVersion: 'v5' });
    await store.refreshAll();
...
describe('Workflow: Multiple instances aggregate', () => {
  ...
  it('refreshAll handles mixed online/offline gracefully', async () => {
    ...
    const store = useInstanceStore();
    store.addInstance({ name: 'Online', url: 'http://online.local', apiToken: 'tok', apiVersion: 'v5' });
    store.addInstance({ name: 'Offline', url: 'http://offline.local', apiToken: 'tok', apiVersion: 'v5' });
    await store.refreshAll();
...
describe('Workflow: Blocking control', () => {
  ...
  it('disable with duration passes seconds to API', async () => {
    const mockGet = vi.fn().mockResolvedValue({ data: onlineSummary({ status: 'disabled' }) });
    vi.mocked(axios.create).mockReturnValue({ get: mockGet, interceptors: { request: { use: vi.fn((fn: unknown) => fn) } } } as never);
    const store = useInstanceStore();
    ...
    const disableCall = mockGet.mock.calls.find(
      ([, cfg]: [unknown, { params?: Record<string, unknown> }]) => cfg?.params?.disable !== undefined,
    );
...
describe('Workflow: Persistence', () => {
  it('instances survive a simulated page reload', () => {
    ...
    store1.addInstance({ name: 'Persistent Pi', url: 'http://pi.hole', apiToken: 'tok', apiVersion: 'v5' });
  ...
  it('activeInstanceId is restored correctly', () => {
    const store1 = useInstanceStore();
    store1.addInstance({ name: 'First',  url: 'http://a.local', apiToken: 'tok', apiVersion: 'v5' });
    store1.addInstance({ name: 'Second', url: 'http://b.local', apiToken: 'tok', apiVersion: 'v5' });
...
describe('Workflow: Gravity update', () => {
  it('updateGravity endpoint is called correctly', async () => {
    const mockGet = vi.fn().mockResolvedValue({ data: { gravity_updated: true } });
    vi.mocked(axios.create).mockReturnValue({ get: mockGet, interceptors: { request: { use: vi.fn((fn: unknown) => fn) } } } as never);
    const { default: PiholeApiService } = await import('@/services/piholeApi');
    await PiholeApiService.updateGravity({ id: 'x', name: 'x', url: 'http://pi.hole', apiToken: 'tok', apiVersion: 'v5', status: 'online', addedAt: '' });
    const call = mockGet.mock.calls.find(
      ([, cfg]: [unknown, { params?: Record<string, unknown> }]) => 'updateGravity' in (cfg?.params ?? {}),
    );
...
describe('Edge cases', () => {
  it('removing the last instance resets activeInstanceId to null', () => {
    const store = useInstanceStore();
    store.addInstance({ name: 'Solo', url: 'http://pi.hole', apiToken: 'tok', apiVersion: 'v5' });
