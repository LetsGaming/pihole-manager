...
function cfg(overrides: Partial<NewInstanceConfig> = {}): NewInstanceConfig {
  return { name: 'Test Pi-hole', url: 'http://192.168.1.100', apiToken: 'abc123', apiVersion: 'v5', ...overrides };
}
...
describe('instanceStore getters', () => {
  it('onlineCount and offlineCount', async () => {
    ...
    vi.mocked(PiholeApiService.getSummary)
      .mockResolvedValue(VALID_SUMMARY); // default: all succeed
    vi.mocked(PiholeApiService.getSummary)
...
describe('instanceStore persistence', () => {
  it('loadFromStorage restores instances', () => {
    localStorage.setItem('orbital_instances', JSON.stringify({
      instances: [{ id: 'ph_abc', name: 'Restored', url: 'http://pi.hole', apiToken: 'tok', apiVersion: 'v5', status: 'unknown', addedAt: '' }],
