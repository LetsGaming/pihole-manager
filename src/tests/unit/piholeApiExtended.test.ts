...
const inst: PiholeInstance = {
  id: 'test', name: 'Test', url: 'http://pi.hole',
  apiToken: 'test-token', apiVersion: 'v5', status: 'online', addedAt: '',
};
...
describe('PiholeApiService.getQueryLog', () => {
  it('parses raw arrays into QueryEntry objects', async () => {
    const payload = { data: [[1700000001, 'A', 'example.com', '192.168.1.10', '2', '0', 'g', '3', '1ms']] };
    const { mockGet } = mockCreate(payload);
    const result = await PiholeApiService.getQueryLog(inst, 100);
    expect(mockGet).toHaveBeenCalledWith('/admin/api.php', expect.objectContaining({ params: expect.objectContaining({ getAllQueries: 100 }) }));
...
describe('PiholeApiService list management', () => {
  ...
  it('addToList posts with add param', async () => {
    ...
    expect(mockPost).toHaveBeenCalledWith('/admin/api.php', null, expect.objectContaining({
  ...
  it('addToList throws when success is false', async () => {
    ...
    await expect(PiholeApiService.addToList(inst, 'black', 'dupe.com')).rejects.toThrow('Already exists');
  });
...
describe('PiholeApiService adlist management', () => {
  it('getAdlists returns data array', async () => {
    mockCreate({ data: [{ id: 1, address: 'https://hosts.file/list.txt', enabled: 1, comment: '' }] });
    expect((await PiholeApiService.getAdlists(inst))[0].address).toBe('https://hosts.file/list.txt');
  });

  it('addAdlist posts to adlist endpoint', async () => {
    const { mockPost } = mockCreate({});
    await PiholeApiService.addAdlist(inst, 'https://new.list/domains.txt', 'New');
    expect(mockPost).toHaveBeenCalledWith('/admin/api.php', null, expect.objectContaining({
      params: expect.objectContaining({ list: 'adlist', add: 'https://new.list/domains.txt' }),
    }));
  });
  ...
  it('removeAdlist posts sub param', async () => {
    ...
    expect(mockPost).toHaveBeenCalledWith('/admin/api.php', null, expect.objectContaining({
      params: expect.objectContaining({ sub: 'https://old.list/domains.txt' }),
    }));
  });

  it('addAdlist throws on failure', async () => {
    mockCreate({}, { success: false, message: 'URL already exists' } as unknown);
    await expect(PiholeApiService.addAdlist(inst, 'https://dupe.list')).rejects.toThrow('URL already exists');
  });
});
...
describe('PiholeApiService.getTopClients', () => {
  it('returns top_sources object', async () => {
    ...
    expect((await PiholeApiService.getTopClients(inst, 5))['10.0.0.1']).toBe(1000);
  });
});
...
describe('PiholeApiService blocking control', () => {
  it('enableBlocking sends enable param', async () => {
    ...
    expect(mockGet).toHaveBeenCalledWith('/admin/api.php', expect.objectContaining({ params: expect.objectContaining({ enable: '' }) }));
  });
  ...
  it('disableBlocking with 0 sends disable:0', async () => {
    ...
    const call = mockGet.mock.calls[0] as [string, { params: Record<string, unknown> }];
...
describe('errorMessage helper', () => {
  it('extracts nested API error message',    () => expect(errorMessage({ response: { data: { error: { message: 'Unauthorized' } } } })).toBe('Unauthorized'));
  it('extracts top-level response message',  () => expect(errorMessage({ response: { data: { message: 'Not found' } } })).toBe('Not found'));
  it('falls back to err.message',            () => expect(errorMessage(new Error('Network error'))).toBe('Network error'));
  it('returns Unknown error for null',       () => expect(errorMessage(null)).toBe('Unknown error'));
  it('returns Unknown error for empty obj',  () => expect(errorMessage({})).toBe('Unknown error'));
});

describe('PiholeApiService.formatUptime', () => {
  it('formats 1d 1h 1m',    () => expect(PiholeApiService.formatUptime(90061)).toBe('1d 1h 1m'));
  it('formats 1h 1m',       () => expect(PiholeApiService.formatUptime(3661)).toBe('1h 1m'));
  it('formats 5m',           () => expect(PiholeApiService.formatUptime(300)).toBe('5m'));
  it('returns <1m',          () => expect(PiholeApiService.formatUptime(30)).toBe('<1m'));
  it('returns null for null',() => expect(PiholeApiService.formatUptime(null)).toBeNull());
  it('formats 2d 3h 15m',    () => expect(PiholeApiService.formatUptime(2*86400+3*3600+15*60)).toBe('2d 3h 15m'));
});
...
describe('PiholeApiService v6 authentication', () => {
  it('v6 authenticates via POST /api/auth and uses X-FTL-SID header', async () => {
    ...
    vi.mocked(axios.create).mockReturnValue({
      post: vi.fn().mockImplementation((_url: string, body: unknown) => {
        ...
        return Promise.resolve({ data: { session: { sid: 'test-session-123', valid: true } } });
      }),
      get: vi.fn().mockImplementation((url: string) => {
        if (url === '/api/stats/summary') return Promise.resolve({ data: {
          queries: { total: 0, blocked: 0, percent_blocked: 0, forwarded: 0, cached: 0 },
          clients: { active: 0, total: 0 },
          gravity: { domains_being_blocked: 0, last_update: 0 },
        }});
        return Promise.resolve({ data: { blocking: true } });
      }),
      interceptors: { request: { use: vi.fn((fn: (c: { headers: Record<string, unknown>; params: Record<string, unknown> }) => unknown) => {
  ...
  it('v5 sets auth param on every request', async () => {
    ...
    vi.mocked(axios.create).mockReturnValue({
      get: vi.fn().mockResolvedValue({ data: { status: 'enabled', dns_queries_today: 0, ads_blocked_today: 0, ads_percentage_today: 0, domains_being_blocked: 0, unique_clients: 0 } }),
      interceptors: { request: { use: vi.fn((fn: (c: { headers: Record<string, unknown>; params: Record<string, unknown> }) => unknown) => {
        const c = { headers: {}, params: {} };
        fn(c);
