...
const inst: Partial<PiholeInstance> = {
  url: "http://pi.hole",
  apiToken: "test-token",
  apiVersion: "v5",
};
...
function mockCreate(
  getResponse: unknown,
  postResponse: unknown = { success: true },
): MockClient {
...
describe("PiholeApiService.getSummary", () => {
  it("fetches summary", async () => {
    ...
    expect(mockGet).toHaveBeenCalledWith("/admin/api.php", {
      params: { summary: "" },
    });
        ...
        return Promise.resolve({
          data: {
            queries: {
              total: 9492,
              blocked: 659,
              percent_blocked: 6.94,
              forwarded: 1624,
              cached: 6721,
            },
            clients: { active: 18, total: 18 },
            gravity: {
              domains_being_blocked: 1950125,
              last_update: 1774168828,
            },
          },
        });
      }
  ...
  it("propagates network errors", async () => {
    ...
    await expect(
      PiholeApiService.getSummary(inst as PiholeInstance),
...
describe("PiholeApiService.getQueryLog", () => {
  it("returns mapped entries", async () => {
    ...
    const result = await PiholeApiService.getQueryLog(
      inst as PiholeInstance,
      100,
    );
  ...
  it("returns empty array when data missing", async () => {
    mockCreate({});
    expect(
      await PiholeApiService.getQueryLog(inst as PiholeInstance, 10),
    ).toEqual([]);
  });
});
...
describe("PiholeApiService.getAdlists", () => {
  it("returns adlist array", async () => {
    mockCreate({
      data: [{ id: 1, address: "https://example.com/list.txt", enabled: 1 }],
    });
...
describe("PiholeApiService.addAdlist", () => {
  ...
  it("throws when success=false", async () => {
    ...
    await expect(
      PiholeApiService.addAdlist(inst as PiholeInstance, "http://dupe.com", ""),
...
describe("PiholeApiService.removeAdlist", () => {
  it("posts with sub param", async () => {
    const { mockPost } = mockCreate({ success: true });
    await PiholeApiService.removeAdlist(
      inst as PiholeInstance,
...
describe("PiholeApiService.getList", () => {
  it("fetches blacklist", async () => {
    ...
    const result = await PiholeApiService.getList(
      inst as PiholeInstance,
      "black",
    );
...
describe("PiholeApiService.addToList", () => {
  it("adds domain to whitelist", async () => {
    ...
    expect(mockPost).toHaveBeenCalledWith("/admin/api.php", null, {
      params: {
        list: "white",
        add: "safe.example.com",
        comment: "always allow",
      },
    });
  });
});
...
describe("PiholeApiService.formatUptime", () => {
  ...
  it("formats only minutes", () =>
    expect(PiholeApiService.formatUptime(300)).toBe("5m"));
...
describe("PiholeApiService.getOverTimeData", () => {
  it("returns domains and ads", async () => {
    const payload = {
      domains_over_time: { "1700000000": 42 },
      ads_over_time: { "1700000000": 5 },
    };
    mockCreate(payload);
...
describe("errorMessage", () => {
  ...
  it("extracts top-level data message", () =>
    expect(errorMessage({ response: { data: { message: "Not found" } } })).toBe(
      "Not found",
    ));
  it("falls back to err.message", () =>
    expect(errorMessage(new Error("went wrong"))).toBe("went wrong"));
  it("returns Unknown error as last resort", () =>
