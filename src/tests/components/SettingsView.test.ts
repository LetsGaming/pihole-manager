...
vi.mock('@/services/piholeApi', () => ({
  default: {
    ...
    testConnection: vi.fn().mockResolvedValue({ ok: true, message: 'Connection successful', latencyMs: 12 }),
...
const STUBS = {
  ...
  'InstanceForm': { template: '<div class="instance-form" />', props: ['isOpen','editing'], emits: ['close','save'] },
};
...
describe('SettingsView', () => {
  ...
  it('openAdd clears editingInstance', () => {
    ...
    store.addInstance({ name: 'Existing', url: 'http://pi.hole', apiToken: 'tok', apiVersion: 'v5' });
  ...
  it('openEdit sets editingInstance and opens form', () => {
    const w = createWrapper();
    const store = useInstanceStore();
    store.addInstance({ name: 'My Pi', url: 'http://pi.local', apiToken: 'tok', apiVersion: 'v6' });
  ...
  it('handleSave with null editingId adds a new instance', () => {
    const w = createWrapper();
    const store = useInstanceStore();
    w.vm.handleSave({ name: 'New Pi', url: 'http://new.pi', apiToken: 'tok', apiVersion: 'v5' }, null);
  ...
  it('handleSave with editingId updates existing instance', () => {
    const w = createWrapper();
    const store = useInstanceStore();
    store.addInstance({ name: 'Old', url: 'http://old.pi', apiToken: 'tok', apiVersion: 'v5' });
    const id = store.instances[0].id;
    w.vm.handleSave({ name: 'Updated', url: 'http://old.pi', apiToken: 'tok', apiVersion: 'v5' }, id);
  ...
  it('handleSave closes the form', () => {
    const w = createWrapper();
    w.vm.showForm = true;
    w.vm.handleSave({ name: 'Pi', url: 'http://pi.hole', apiToken: 'tok', apiVersion: 'v5' }, null);
    expect(w.vm.showForm).toBe(false);
  });
  ...
  it('handleSave shows success notification', () => {
    ...
    w.vm.handleSave({ name: 'Pi X', url: 'http://pi.x', apiToken: 'tok', apiVersion: 'v5' }, null);
  ...
  it('confirmRemove sets pendingRemove and opens alert', () => {
    const w = createWrapper();
    const store = useInstanceStore();
    store.addInstance({ name: 'To Remove', url: 'http://pi.hole', apiToken: 'tok', apiVersion: 'v5' });
    const inst = store.instances[0];
  ...
  it('save() persists settings to localStorage', () => {
    ...
    w.vm.save();
    const saved = JSON.parse(localStorage.getItem('orbital_app_settings') ?? '{}');
  ...
  it('clearAll removes all instances after confirm', () => {
    const w = createWrapper();
    const store = useInstanceStore();
    store.addInstance({ name: 'A', url: 'http://a.local', apiToken: 'tok', apiVersion: 'v5' });
    store.addInstance({ name: 'B', url: 'http://b.local', apiToken: 'tok', apiVersion: 'v5' });
  ...
  it('clearAll does nothing if confirm returns false', () => {
    const w = createWrapper();
    const store = useInstanceStore();
    store.addInstance({ name: 'Keep', url: 'http://pi.hole', apiToken: 'tok', apiVersion: 'v5' });
