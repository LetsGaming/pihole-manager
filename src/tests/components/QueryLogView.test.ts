...
const STUBS = {
  ...
  'EmptyState':      { template: '<div class="empty-state">{{ title }}</div>', props: ['icon','title','subtitle'] },
  'QueryLogToolbar': {
...
describe('QueryLogView', () => {
  ...
  it('filteredEntries filters by status=blocked', async () => {
    ...
    expect(w.vm.filteredEntries.every((e: { status: string }) => e.status === 'blocked')).toBe(true);
  });
  ...
  it('filteredEntries filters by status=allowed', async () => {
    ...
    expect(w.vm.filteredEntries.every((e: { status: string }) => e.status === 'allowed')).toBe(true);
  });
  ...
  it('filteredEntries filters by domain search', async () => {
    ...
    expect(w.vm.filteredEntries.every((e: { domain: string }) => e.domain.includes('example'))).toBe(true);
  });
  ...
  it('filteredEntries filters by client search', async () => {
    ...
    expect(w.vm.filteredEntries.some((e: { client: string }) => e.client === '192.168.1.10')).toBe(true);
  });
  ...
  it('pagedEntries slices at PAGE_SIZE (50)', () => {
    ...
    w.vm.entries = Array.from({ length: 60 }, (_, i) => ({
      timestamp: i, domain: `d${i}.com`, client: '10.0.0.1',
      type: 'A', status: 'allowed', statusCode: 2,
      _instanceId: 'test', _instanceName: 'Test', _key: `k-${i}`,
    }));
