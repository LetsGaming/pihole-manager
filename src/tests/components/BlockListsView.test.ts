...
vi.mock("@/services/piholeApi", () => ({
  default: {
    ...
    getAdlists: vi.fn().mockResolvedValue([
      ...
      {
        id: 2,
        address: "https://another.list/ads.txt",
        enabled: 0,
        comment: "",
      },
    ]),
    ...
    getList: vi.fn().mockImplementation((_inst: unknown, type: string) => {
      if (type === "black")
        return Promise.resolve([
          { id: 1, domain: "evil.com", enabled: 1, comment: "" },
        ]);
      if (type === "white")
        return Promise.resolve([
          { id: 2, domain: "safe.example.com", enabled: 1, comment: "" },
        ]);
      return Promise.resolve([]);
    }),
...
const STUBS = {
  ...
  InstanceTabBar: {
    template: '<div class="instance-tabs" />',
    props: ["instances", "selectedId"],
    emits: ["select"],
  },
  AddDomainForm: {
  ...
  DomainListTable: {
    template: '<div class="domain-table">{{ title }}</div>',
    props: ["title", "entries", "searchQuery", "loading"],
