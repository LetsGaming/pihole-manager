  ...
  default: {
    ...
    getTopDomains: vi
      ...
      .mockResolvedValue({
        ...
        topBlocked: { "doubleclick.net": 300 },
      }),
    getTopClients: vi
      .fn()
...
const STUBS = {
  ...
  StatCard: {
    template: '<div class="stat-card">{{ label }}</div>',
    props: ["label", "value", "accent"],
  },
  TopDomainsBar: {
    template: '<div class="top-bar">{{ domain }}</div>',
    props: ["domain", "count", "width", "variant"],
  },
};
