...
vi.mock('@/services/hardwareService', () => ({
  default: {
    getHardwareInfo: vi.fn().mockResolvedValue({
      cpuLoad: 45.2, cpuTemp: 52.0, cpuModel: 'ARMv7', cpuCores: 4,
      memTotal: 1073741824, memUsed: 536870912, memFree: 536870912, memPercent: 50,
      diskTotal: 16000000000, diskUsed: 8000000000, diskPercent: 50,
      hostname: 'raspberrypi', ipAddress: '192.168.1.100', interface: 'eth0',
      uptimeSeconds: 86400, uptimeFormatted: '1d',
      piholeVersion: 'v5.17.1', ftlVersion: 'v5.23', webVersion: 'v5.20.1',
      domainsBlocked: 90000, gravityLastUpdate: null,
    }),
    formatBytes:   (b: number | null) => b ? `${Math.round(b / 1048576)} MB` : null,
    formatTemp:    (c: number | null) => c != null ? `${c.toFixed(1)} °C` : null,
    severityClass: (p: number) => p >= 90 ? 'crit' : p >= 70 ? 'warn' : 'normal',
    tempSeverity:  (c: number) => c >= 75 ? 'crit' : c >= 60 ? 'warn' : 'normal',
  },
}));
...
const STUBS = {
  ...
  'HardwareCard':    { template: '<div class="hw-card" />', props: ['instance','hwData','loading'] },
};
...
describe('HardwareView', () => {
  ...
  it('renders a HardwareCard for each instance', async () => {
    ...
    store.addInstance({ name: 'Pi Alpha', url: 'http://pi.a', apiToken: 'tok', apiVersion: 'v5' });
    store.addInstance({ name: 'Pi Beta',  url: 'http://pi.b', apiToken: 'tok', apiVersion: 'v5' });
    await w.vm.$nextTick();
  ...
  it('calls getHardwareInfo on mount for each instance', async () => {
    vi.mocked(HardwareService.getHardwareInfo).mockResolvedValue({
      cpuLoad: 20, cpuTemp: null, cpuModel: null, cpuCores: null,
    ...
    const _w = mount(HardwareView, { global: { plugins: [pinia], stubs: STUBS } });
  ...
  it('stores hwData keyed by instance id after fetch', async () => {
    ...
    setActivePinia(pinia);
    const store = useInstanceStore();
    const inst = store.addInstance({ name: 'Pi', url: 'http://pi.hole', apiToken: 'tok', apiVersion: 'v5' });
    const w = mount(HardwareView, { global: { plugins: [pinia], stubs: STUBS } });
