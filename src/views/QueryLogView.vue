...
const ACCESSORS: Partial<
  Record<string, (e: EnrichedQueryEntry) => string | number>
> = {
...
export default defineComponent({
  ...
  methods: {
    ...
    async fetchLog(): Promise<void> {
      const targets =
        this.selectedInstanceId === "all"
          ? this.instanceStore.instances.filter(
              (i) => i.status === "online" || i.status === "unknown",
            )
          : this.instanceStore.instances.filter(
      ...
      try {
        ...
        results.forEach((r) => {
          if (r.status === "fulfilled") combined.push(...r.value);
            ...
            notifications.error(
              `Query log: ${(r.reason as Error)?.message ?? "Fetch failed"}`,
            );
        });
        ...
        for (let i = 0; i < combined.length; i++) {
          combined[i]._key =
            `${combined[i]._instanceId}-${combined[i].timestamp}-${combined[i].domain}-${i}`;
        }
        ...
        useNotificationStore().error(
          `Failed to fetch log: ${(err as Error).message}`,
        );
      } finally {
    ...
    async whitelistDomain(domain: string, instanceId: string): Promise<void> {
      const inst = this.instanceStore.instances.find(
        (i) => i.id === instanceId,
      );
      if (!inst) return;
      ...
      try {
        ...
        n.success(`Whitelisted: ${domain}`);
      } catch (err) {
        n.error(`Failed: ${(err as Error).message}`);
      }
    },
