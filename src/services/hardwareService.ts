...
const HardwareService = {
  async getHardwareInfo(instance: PiholeInstance): Promise<HardwareInfo> {
    ...
    return {
      cpuLoad: HardwareService._parseCpuLoad(
      ...
      diskPercent:
        sysData.disk_percent != null ? parseFloat(sysData.disk_percent) : null,
      ...
      domainsBlocked:
        (sumData as Record<string, unknown>).domains_being_blocked != null
          ? Number((sumData as Record<string, unknown>).domains_being_blocked)
          : null,
      gravityLastUpdate: (sumData as Record<string, unknown>)
        .gravity_last_updated
        ? (() => {
            const ts = (
              (sumData as Record<string, unknown>)
                .gravity_last_updated as Record<string, unknown>
            )?.absolute;
