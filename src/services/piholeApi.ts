...
const STATUS_MAP: Record<number, QueryStatus> = {
  1: 'blocked',  2: 'allowed',  3: 'cached',   4: 'blocked',
  5: 'blocked',  6: 'blocked',  7: 'allowed',  8: 'allowed',
  9: 'blocked', 10: 'allowed', 11: 'allowed', 12: 'blocked',
 13: 'blocked', 14: 'blocked', 15: 'blocked', 16: 'cached',
};
...
interface CachedSession { sid: string; expires: number }
...
function v5Client(instance: PiholeInstance): AxiosInstance {
  const client = axios.create({ baseURL: baseURL(instance.url), timeout: 10_000 });
...
async function ensureV6Session(instance: PiholeInstance): Promise<string> {
  ...
  try {
    sessionStorage.setItem(
      ...
      JSON.stringify({ sid, expires: Date.now() + ttl } satisfies CachedSession),
    );
  } catch {
...
function v6QueryStatus(status: string | number): QueryStatus {
  if (typeof status === 'string') {
    ...
    // Blocked categories
    if (s.includes('GRAVITY') || s.includes('REGEX') || s.includes('DENYLIST') ||
        s.includes('BLOCKED') || s === 'SPECIAL_DOMAIN') return 'blocked';
    // Cached categories
    if (s === 'CACHE' || s === 'CACHE_STALE' || s === 'RETRIED' || s === 'RETRIED_DNSSEC') return 'cached';
    // Unknown / in-flight
    if (s === 'IN_PROGRESS' || s === 'DBBUSY' || s === 'UNKNOWN') return 'unknown';
    // Forwarded and everything else = allowed
    return 'allowed';
  }
  // v5-compat numeric fallback
  if ([1, 4, 5, 6, 7, 8, 9, 12, 13, 14, 15].includes(status as number)) return 'blocked';
...
const PiholeApiService = {
    ...
    if (instance.apiVersion === 'v6') {
      return v6Call(instance, async (client) => {
        ...
        const [summaryRes, blockingRes] = await Promise.all([
          ...
          client.get<{ blocking: boolean }>('/api/dns/blocking').catch(() => ({ data: { blocking: true } })),
        ]);
        const d = summaryRes.data;
        return {
          status:                d.gravity !== undefined   // gravity only absent on error
            ? (blockingRes.data.blocking ? 'enabled' : 'disabled')
            : 'enabled',
          ...
          ...(d.gravity.last_update != null && {
            gravity_last_updated: { absolute: d.gravity.last_update, relative: { days: 0, hours: 0, minutes: 0 } },
          }),
        } as PiholeSummary;
      });
    }
    const { data } = await v5Client(instance).get<PiholeSummary>('/admin/api.php', {
      params: { summary: '' },
    });
    return data;
  },

  async getStatus(instance: PiholeInstance): Promise<{ status: BlockingStatus }> {
    if (instance.apiVersion === 'v6') {
      return v6Call(instance, async (client) => {
        const { data } = await client.get<{ blocking: boolean }>('/api/dns/blocking');
        return { status: data.blocking ? 'enabled' : 'disabled' };
      });
    }
    const { data } = await v5Client(instance).get<{ status: BlockingStatus }>('/admin/api.php', {
      params: { status: '' },
    });
    return data;
  },
  ...
  async enableBlocking(instance: PiholeInstance): Promise<{ status: BlockingStatus }> {
    if (instance.apiVersion === 'v6') {
      return v6Call(instance, async (client) => {
    ...
    if (data.status !== 'enabled') throw new Error(`Unexpected status: ${data.status}`);
    return data;
  },

  async disableBlocking(instance: PiholeInstance, seconds = 0): Promise<{ status: BlockingStatus }> {
    if (instance.apiVersion === 'v6') {
      return v6Call(instance, async (client) => {
    ...
    if (data.status !== 'disabled') throw new Error(`Unexpected status: ${data.status}`);
    return data;
  },

  // ─── Query Log ─────────────────────────────────────────────────────────────

  async getQueryLog(instance: PiholeInstance, count = 100): Promise<QueryEntry[]> {
    if (instance.apiVersion === 'v6') {
      return v6Call(instance, async (client) => {
    ...
    const { data } = await v5Client(instance).get<{ data?: unknown[][] }>('/admin/api.php', {
  ...
  async getTopDomains(instance: PiholeInstance, count = 10): Promise<TopDomainsResult> {
    if (instance.apiVersion === 'v6') {
      return v6Call(instance, async (client) => {
        const { data } = await client.get<{
        ...
        const toMap = (arr: Array<{ domain: string; count: number }> = []): TopDomainsMap =>
          Object.fromEntries(arr.map((e) => [e.domain, e.count]));
        return { topDomains: toMap(data.top_domains), topBlocked: toMap(data.top_blocked) };
      });
    }
    const { data } = await v5Client(instance).get<{
      top_queries?: TopDomainsMap; top_ads?: TopDomainsMap;
    }>('/admin/api.php', { params: { topItems: count } });
    return { topDomains: data.top_queries ?? {}, topBlocked: data.top_ads ?? {} };
  },

  async getTopClients(instance: PiholeInstance, count = 10): Promise<TopClientsMap> {
    if (instance.apiVersion === 'v6') {
      return v6Call(instance, async (client) => {
        const { data } = await client.get<{
    ...
    const { data } = await v5Client(instance).get<{ top_sources?: TopClientsMap }>(
  ...
  async getOverTimeData(instance: PiholeInstance): Promise<OverTimeData> {
    ...
    return { domains: data.domains_over_time ?? {}, ads: data.ads_over_time ?? {} };
  },
  ...
  async getAdlists(instance: PiholeInstance): Promise<Adlist[]> {
    if (instance.apiVersion === 'v6') {
      return v6Call(instance, async (client) => {
        const { data } = await client.get<{
          lists?: Array<{ id: number; address: string; enabled: boolean; comment: string; number?: number }>;
    ...
    const { data } = await v5Client(instance).get<{ data?: Adlist[] }>('/admin/api.php', {
      params: { list: 'adlist' },
    });
    return data.data ?? [];
  },

  async addAdlist(instance: PiholeInstance, url: string, comment = ''): Promise<void> {
    if (instance.apiVersion === 'v6') {
      return v6Call(instance, async (client) => {
        const { data } = await client.post<{ error?: { message?: string } }>(
          '/api/lists', { address: url, comment, type: 'block', enabled: true },
        );
        if (data.error) throw new Error(data.error.message ?? 'Failed to add adlist');
      });
    }
    const { data } = await v5Client(instance).post<{ success?: boolean; message?: string }>(
      '/admin/api.php', null, { params: { list: 'adlist', add: url, comment } },
    );
    if (data.success === false) throw new Error(data.message ?? 'Failed to add adlist');
  },
  ...
  async removeAdlist(instance: PiholeInstance, url: string): Promise<void> {
    ...
    if (data.success === false) throw new Error(data.message ?? 'Failed to remove adlist');
  },
  ...
  _v6ListType(listType: DomainListType): { type: string; kind: string } {
    switch (listType) {
      case 'black':       return { type: 'deny',  kind: 'exact' };
      case 'white':       return { type: 'allow', kind: 'exact' };
      case 'regex_black': return { type: 'deny',  kind: 'regex' };
      case 'regex_white': return { type: 'allow', kind: 'regex' };
    }
  },

  async getList(instance: PiholeInstance, listType: DomainListType): Promise<DomainEntry[]> {
    if (instance.apiVersion === 'v6') {
      const { type, kind } = PiholeApiService._v6ListType(listType);
      return v6Call(instance, async (client) => {
        const { data } = await client.get<{
          domains?: Array<{ id: number; domain: string; enabled: boolean; comment: string }>;
    ...
    const { data } = await v5Client(instance).get<{ data?: DomainEntry[] }>('/admin/api.php', {
  ...
  async addToList(
    instance: PiholeInstance, listType: DomainListType, domain: string, comment = '',
  ): Promise<void> {
    if (instance.apiVersion === 'v6') {
      const { type, kind } = PiholeApiService._v6ListType(listType);
      return v6Call(instance, async (client) => {
        const { data } = await client.post<{ error?: { message?: string } }>(
          '/api/domains', { domain, comment, type, kind, enabled: true },
        );
        if (data.error) throw new Error(data.error.message ?? 'Failed to add domain');
      });
    }
    const { data } = await v5Client(instance).post<{ success?: boolean; message?: string }>(
      '/admin/api.php', null, { params: { list: listType, add: domain, comment } },
    );
    if (data.success === false) throw new Error(data.message ?? 'Failed to add domain');
  },

  async removeFromList(
    instance: PiholeInstance, listType: DomainListType, domain: string,
  ): Promise<void> {
    if (instance.apiVersion === 'v6') {
      const { type, kind } = PiholeApiService._v6ListType(listType);
      return v6Call(instance, async (client) => {
        // v6: DELETE /api/domains/{type}/{kind}/{domain}
        await client.delete(`/api/domains/${type}/${kind}/${encodeURIComponent(domain)}`);
      });
    }
    const { data } = await v5Client(instance).post<{ success?: boolean; message?: string }>(
      '/admin/api.php', null, { params: { list: listType, sub: domain } },
    );
    if (data.success === false) throw new Error(data.message ?? 'Failed to remove domain');
  },
  ...
  async updateGravity(instance: PiholeInstance): Promise<void> {
    ...
    await v5Client(instance).get('/admin/api.php', { params: { updateGravity: '' } });
  },
  ...
  async getVersions(instance: PiholeInstance): Promise<PiholeVersions> {
    if (instance.apiVersion === 'v6') {
      return v6Call(instance, async (client) => {
        ...
        return {
          core_current: data.version?.core?.local?.version
                     ?? data.version?.core?.tag
                     ?? undefined,
          FTL_current:  data.version?.FTL?.local?.version
                     ?? data.version?.FTL?.tag
                     ?? undefined,
          web_current:  data.version?.web?.local?.version
                     ?? data.version?.web?.tag
                     ?? undefined,
        };
      });
    }
    const { data } = await v5Client(instance).get<PiholeVersions>('/admin/api.php', {
      params: { versions: '' },
    });
    return data;
  },
  ...
  async getSystemInfo(instance: PiholeInstance): Promise<Record<string, string>> {
    if (instance.apiVersion === 'v6') {
      try {
        return await v6Call(instance, async (client) => {
          ...
          const cpuPercent = sys.cpu?.['%cpu'] != null
            ? String(sys.cpu['%cpu'])
            : Array.isArray(sys.cpu?.load?.percent) && sys.cpu.load.percent.length
              ? String(sys.cpu.load.percent[0])
              : undefined;

          // Temperature from sensors array (°C sensor)
          const tempSensor = sys.sensors?.find((s) =>
            s.name.toLowerCase().includes('cpu') && s.prefix === '°C',
          ) ?? sys.sensors?.find((s) => s.prefix === '°C');

          // Memory percent: use '%used' field directly
          const memPercent = sys.memory?.ram?.['%used'] != null
          ...
          return {
            ...
            ...(sys.cpu?.nprocs != null                && { cpu_cores:    String(sys.cpu.nprocs) }),
            // v6 memory values are in KB — multiply by 1024 to get bytes for formatBytes()
            ...(sys.memory?.ram?.total != null         && { mem_total:    String(sys.memory.ram.total * 1024) }),
            ...(sys.memory?.ram?.used  != null         && { mem_used:     String(sys.memory.ram.used  * 1024) }),
            ...(sys.memory?.ram?.free  != null         && { mem_free:     String(sys.memory.ram.free  * 1024) }),
            // Round %used to one decimal place
            ...(memPercent !== undefined               && { mem_percent:  String(parseFloat(memPercent).toFixed(1)) }),
    ...
    try {
      const { data } = await v5Client(instance).get<Record<string, string>>(
        '/admin/api_FTL.php', { params: { getSysInfo: '' } },
      );
      return data ?? {};
    } catch {
      return {};
  ...
  async testConnection(instance: PiholeInstance): Promise<ConnectionTestResult> {
    const start = Date.now();
    try {
      const data = await PiholeApiService.getSummary(instance);
      const latencyMs = Date.now() - start;
      if (!data || data.status === undefined) {
        return { ok: false, message: 'Connected but invalid response — check credentials', latencyMs };
      }
